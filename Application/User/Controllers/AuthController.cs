...
public class AuthController(IAuthRepository repository) : ControllerBase
{
    ...
    {
        ApiResponse<string> response = await repository.SendOtp(dto, Enumerations.UserRole.User, false);
    ...
    {
        ApiResponse<string> response = await repository.SendOtp(dto, Enumerations.UserRole.User, true);
        return StatusCode(response.StatusCode, response);
    }
    ...
    {
        ApiResponse<VerifyOtpReadDto> response = await repository.VerifyOtp(dto, Enumerations.UserRole.User);
        return StatusCode(response.StatusCode, response);
    }
    ...
    {
        ApiResponse<RefreshTokenReadDto> response = await repository.RefreshToken(dto, Enumerations.UserRole.User);
        return StatusCode(response.StatusCode, response);
    }
}